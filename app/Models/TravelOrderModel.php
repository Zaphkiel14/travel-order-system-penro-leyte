...
class TravelOrderModel extends Model
{
   ...
   function displayAllTravelOrders()
   {
      $builder = $this->db->table('travel_orders');
      $builder->select('travel_orders.travel_order_id, travel_orders.travel_order_number, travel_orders.destination, travel_orders.departure_date,travel_orders.arrival_date, travel_orders.status, users.first_name, users.last_name, travel_orders.created_at');
      $builder->join('users', 'travel_orders.user_id = users.user_id');
      $query = $builder->get();
